...
layui.define(["element", "jquery"], function (exports) {
            ...
            if (data[i].children != undefined && data[i].children.length > 0) {
                ...
                for (var j = 0; j < data[i].children.length; j++) {
                    if (data[i].children[j].target == "_blank") {
                        ulHtml += '<dd><a data-url="' + _ctx  + data[i].children[j].href + '" target="' + data[i].children[j].target + '">';
                    } else {
                        ulHtml += '<dd><a data-url="' + _ctx  +  data[i].children[j].href + '">';
                    }
                ...
                if (data[i].target == "_blank") {
                    ulHtml += '<a data-url="' + _ctx + data[i].href + '" target="' + data[i].target + '">';
                } else {
                    ulHtml += '<a data-url="' + _ctx + data[i].href + '">';
                }
                if (data[i].icon != undefined && data[i].icon != '') {
                    if (data[i].icon.indexOf("icon-") != -1) {
                        ulHtml += '<i class="seraph ' + data[i].icon + '" data-icon="' + data[i].icon + '"></i>';
    ...
    $(".refresh").on("click", function () {
        if ($(this).hasClass("refresh-this")) {
            $(this).removeClass("refresh-this");
            $(this).addClass('layui-btn-disabled');
            $(".clildFrame .layui-tab-item.layui-show").find("iframe")[0].contentWindow.location.reload();
            setTimeout(function () {
                $(".refresh").addClass("refresh-this");
